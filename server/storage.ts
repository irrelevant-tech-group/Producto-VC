...
import {
  ...
  funds, Fund, InsertFund // Importando las nuevas tablas y tipos
} from "@shared/schema";
import { 
  DashboardMetrics, 
  StartupSummary, 
...
export interface IStorage {
  ...
  getFunds(): Promise<Fund[]>; // Nuevo método para listar todos los fondos
  ...
  getStartupSummaries(fundId?: string): Promise<StartupSummary[]>; // Modificado para filtrar por fondo
  ...
  searchChunks(query: string, startupId?: string, limit?: number, fundId?: string): Promise<Chunk[]>; // Modificado
  
  // Búsqueda vectorial
  searchChunksByEmbedding(embedding: number[], startupId?: string, limit?: number, fundId?: string): Promise<Chunk[]>; // Modificado
  ...
  // Activity operations
  getRecentActivities(limit?: number, fundId?: string): Promise<ActivityItem[]>; // Modificado
  createActivity(activity: InsertActivity): Promise<Activity>;
  
  // Dashboard operations
  getDashboardMetrics(fundId?: string): Promise<DashboardMetrics>; // Modificado
  ...
  getPopularQuestions(limit?: number, fundId?: string): Promise<Array<{question: string; count: number}>>; // Modificado
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Nuevos métodos para usuarios con Clerk
  ...
  // Startup operations - ACTUALIZADOS para considerar fundId
  ...
  // ACTUALIZADO - Mapear nuevos campos al INSERT considerando fundId
  ...
  // ACTUALIZADO - Incluir filtrado por fundId
    ...
    try {
      ...
      for (const { startup, docCount } of startupsWithDocs) {
        ...
        summaries.push({
          ...
          fundId: startup.fundId || null, // Añadimos fundId a la respuesta
        });
      }
  ...
  // Document operations (actualizados para considerar fundId)
  ...
  // Chunk operations (actualizados para considerar fundId)
  ...
  // Actualizado para incluir fundId
  ...
  // Activity operations (actualizado para considerar fundId)
  async getRecentActivities(limit = 10, fundId?: string): Promise<ActivityItem[]> {
    ...
    return results.map(activity => ({
      ...
      fundId: activity.fundId || undefined, // Añadimos fundId a la respuesta
    }));
  }
  ...
  // Dashboard operations (actualizado para considerar fundId)
  ...
  // NUEVAS operaciones para consultas AI con persistencia (actualizado para fundId)
